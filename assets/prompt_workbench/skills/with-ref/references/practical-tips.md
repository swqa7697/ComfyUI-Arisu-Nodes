...
- H3 Motion Context is opt-in by `motion.present` in MCP. Continuation rules live in `motion-context.md`. Closing state = `motion.stills` + `motion.notes`. On a Hybrid continuation clip, treat `keyframes.first` as absent; `keyframes.last` may stay.
