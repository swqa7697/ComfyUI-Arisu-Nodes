...
- **Server-owned authorization.** Treat workflow values, linked inputs, query parameters, request bodies and browser bookmarks as untrusted. Image roots come only from the built-in input/output directories and the startup snapshot of administrator-owned `arisu_paths.json` beside the pack entrypoint (`common/paths.py`). Never let an HTTP route, workflow or ComfyUI user setting add roots, choose the configuration file, or supply a base directory. Missing/invalid configuration grants no external roots; configuration changes require restart. Keep the local file out of Git and published artifacts.
- **UI restrictions are not authorization.** Load Image's location widgets are persisted but hidden and socketless; Browse owns UI selection. Remove restored location wires with a reselection warning, rather than silently using stale widget values. Direct API and workflow JSON inputs remain untrusted and must pass the same server-side checks. Never relax containment because an input is hidden or cannot be wired in the normal UI. Preview & Save prefixes remain relative to output, independently of configured image-read roots; temporary previews remain beneath temp.
...
- **Decode raster content only.** Extensions and MIME guesses are filters, not proof of file contents. Use the shared restricted raster opener for node loading and view rendering; preview saves accept PNG only. Never enable document/vector interpreters for renamed files. Serve decoded PNG/WebP pixels with source metadata removed and `X-Content-Type-Options: nosniff`, not raw files or automatically selected compressed sidecars. Refuse SVG and disguised EPS/WMF. Preserve intended workflow metadata in saved PNGs.
