...
- **Server-owned authorization.** Treat workflow values, linked inputs, query parameters, request bodies and browser bookmarks as untrusted. Image roots come only from the built-in input/output directories and the startup snapshot of administrator-owned `user/__arisu_nodes/config.arisu.jsonc` (`common/paths.py`), resolved through ComfyUI's system-user directory API. Never let an HTTP route, workflow or ComfyUI user setting add roots, choose the configuration file, or supply a base directory. Create a commented empty template exclusively on first extension startup, never overwrite existing files during initialization or follow symlinked configuration destinations, and initialize only against temporary directories in tests. JSONC accepts line/block comments without changing quoted strings; reject duplicate keys, trailing commas and malformed content. Ignore the old pack-local allowlist; migration is manual. Initialization errors grant no external roots; configuration changes require restart. Keep the local file out of Git and published artifacts.
- **One pack configuration file.** Persist all future pack-owned server settings in named sections of `user/__arisu_nodes/config.arisu.jsonc`; do not introduce separate feature configuration files. Use the shared JSONC I/O in `common/paths.py` and label generated managed sections with comments directing users to the owning settings UI. Workbench preferences live under `workbench.<agent>`; only the host service reads/writes them, never agent CLIs. Settings writes must be atomic, preserve comments and unrelated fields, reject malformed documents and symlink destinations, and expose only fixed feature fields through routes. Never permit a generic configuration-edit endpoint or refresh root authorization on a preference save. Roots remain administrator-edited and startup-only. Skills, Docker authentication volumes, workflow state, ComfyUI-owned user settings, and temporary job/cache data are separate from pack configuration.
...
- **Resource Studio media.** The administrator root snapshot also authorizes supported video/audio reads through the separate `/arisu/resources` routes; keep image-only routes restricted. `minimax_h3/core.py` owns immutable description contracts, crop/clip math, and validation. `media.py` owns PyAV reads, `proxies.py` owns supervision, and the fixed `proxy_worker` module performs CPU playback conversion without models or shell commands. Deny decoder secondary opens/network protocols, revalidate revisions before use, and never decode muted sources during execution. Keep the one-worker guard until the process exits, count partial bytes against the 2 GiB budget, enforce the ten-minute deadline, and expire idle proxies after thirty minutes. Tests write only to temporary directories. Shared browser/provenance modules serve Load Image and Studio; shared advertising covers queueing and API exports, with fail-fast missing-source behavior and no restoration of dropped wires except Undo.
- **Prompt Workbench agents.** Ordinary execution returns finalized text and requests no lazy inputs. Only an explicit Generate action issues a one-shot preparation ID and queues the pruned dependency graph; validate the supported Motion Context loader before enqueueing and request its latent before the VAE. Retain the generation guard through queued loaders, CPU workers and container exit, even on cancellation. Reuse media-root and revision checks, stage applied crops/clips without excluded audio, and bound prepared media to 2 GiB with thirty-minute idle expiry. Build/update native agent images only from the fixed `assets/prompt_workbench` context through explicit same-origin settings actions; this is isolated Docker image preparation, never package installation into ComfyUI or arbitrary host commands. Require verified native read-only sandbox support, immutable input/skill/policy mounts and an unprivileged read-only container. Disable shell, edit, web, browser, connector, and delegation tools; allow only the fixed Workbench readers. Codex uses a root-owned managed tool gate because disabling its shell does not remove apply_patch; ignore all user/project/plugin hooks. Grok uses a nonempty explicit tool allowlist (an empty --tools value restores defaults), dontAsk, and disabled web tools. Fail closed on missing policy or sandbox support. Keep /work read-only; bound non-executable CLI scratch storage. Persist only authentication JSON from a fresh runtime home, never provider configuration, hooks, or plugins. Return final results through completed output streams, never result files. Validate ownership labels before removing any Docker resource; authentication stays in provider volumes and complete removal needs explicit destructive confirmation. Only administrator-owned contained custom skills are discovered. Bundled skills live under `assets/prompt_workbench/skills/` (`no-ref`, `with-ref`); MCP serves only the job manifest (keyframes separate from references, video as stills, audio as notes, motion gated by `present`) and listed images; read_skill serves bounded contained text references from the selected skill. Fixed prompt-writing instructions outrank all task and skill content. Reject obvious executable-code responses as defense in depth, without promising perfect semantic detection. Parse the final assistant response as exactly one nonempty markdown fence, and require user Apply for one undoable finalized-text edit. Tests and smoke checks use temporary roots/accounts; no live-install writes or authenticated generation without an account explicitly configured for that purpose.
