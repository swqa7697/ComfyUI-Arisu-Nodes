...
const prototypes = {};
for (const name of [SETTINGS, SETTINGS_UPSCALE, HYBRID, HYBRID_ADVANCED, 'ArisuMiniMaxH3ResourceStudio', 'ArisuMiniMaxH3PromptWorkbench']) {
...
test('socket vetoes and serialization preserve advertised and explicit sources', async () => {
  ...
  }
  {
    ...
    const workbench = makeNode({
      id: 13,
      type: 'ArisuMiniMaxH3PromptWorkbench',
      graph: root,
      inputs: [{ name: 'video_settings' }, { name: 'resources' }, { name: 'context_latent' }, { name: 'vae' }],
      widgets: [{ name: 'finalized_prompt', value: 'keep' }],
    });
    prototypes[workbench.type].onAdded.call(workbench);
    app.promptForTest.output[13] = { class_type: workbench.type, inputs: { finalized_prompt: 'keep' } };
    ...
    assert.deepEqual(exported.output[13].inputs, { finalized_prompt: 'keep', resources: ['10', 0] });
    assert.equal(workbench.widgets[0].value, 'keep');
    assert.equal(workbench.inputs.find((input) => input.name === 'resources').disabled, true);
    assert.deepEqual(
