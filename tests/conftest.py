...
2. Wire ``sys.path``: the repo root first, so ``src.arisu_nodes`` and ``tests``
   resolve to this repo, then ComfyUI's root right after it when
   ``COMFYUI_PATH`` is set, so ``comfy_api`` resolves. ComfyUI's tree has its
   own top-level ``tests`` package, hence the ordering.
"""
...
if COMFYUI_PATH and COMFYUI_PATH not in sys.path:
    sys.path.insert(sys.path.index(REPO_ROOT) + 1, COMFYUI_PATH)
