...
- Add MiniMax H3 Model Loader with `native` loading and experimental `hybrid` AdaLN replacement over an inclusive block range, an independent final-AdaLN toggle, source-checked native reload support, mode-first controls, and visible disabled `hybrid` settings in `native` mode; preserve model selections and node size across mode changes and workflow restoration, and update the example workflow to use the new loader.
...
- Treat Motion Context continuation clips as missing the first-frame lock in bundled Prompt Workbench skills, keep unfinished motion through the airlock, and invent a full clip story from a thin brief.
- Preserve the expanded or collapsed Motion Context section in Prompt Workbench across panel refreshes and connection changes.
...
- Resolve same-graph Set/Get routes for settings ownership and Workbench reference notes, refresh source changes, and reject stale or broken routed bundles.
- Fix Prompt Workbench generation with Get nodes and root-to-subgraph connections by resolving their upstream sources through a shared execution map.
...
- Use consistent capitalization for node buttons, dialog fields, headings, and UI labels while preserving lowercase wired input/output labels and existing workflow values.
