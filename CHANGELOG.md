...
- Add the `ArisuPathBuilder` node (category `Arisu Nodes/Common`): join separate text fields into one `/`-separated path for `filename_prefix` inputs, with a `+ field` / `- field` button row to show up to 16 fields.
