...
to get `length` on its 17k+5 frame grid. The values come out as plain numbers and as one `settings`
bundle the **MiniMax H3 Hybrid to Video** nodes accept.

Turn `advertise` on (it is off by default) and every hybrid node in the same graph that has no
`settings` link takes the bundle automatically: its own `width` / `height` / `length` widgets grey out
the moment the switch flips, and a hybrid node added later arrives greyed.
...
| `advertise`    | BOOLEAN | Drive every unlinked hybrid node in this graph (default off); their widgets grey out at once. Read by the frontend only. |
...
| `settings` | ARISU_MINIMAX_H3_SETTINGS | The three values as one bundle for the hybrid nodes' `settings` input. |

The plain outputs are always available, whether or not the node advertises.

## Wiring

```
Video Settings ─ settings ──▶ (settings) Hybrid to Video        explicit link, works anywhere
               ├ width ─────▶ any INT input
               └ length ────▶ any INT input

Video Settings (advertise on)   ...   Hybrid to Video           no link: the bundle is injected at queue time
```

## Notes

- Advertising covers the root graph only. Inside a subgraph, link `settings` explicitly; the link
  works everywhere.
...
- A hybrid node with a `settings` link ignores advertising and follows its link.
- Greyed widgets keep their old numbers but they are not used; the bundle always wins. If a greyed
  widget was converted to an input and wired from elsewhere, remove that link, it is overridden too.
- The greying updates when the switch flips, when a node is added or removed, when a `settings` link
  changes, and when a workflow loads.
